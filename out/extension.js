...
function activate(context) {
    ...
    let editConfigsCmd = vscode.commands.registerCommand('cpp-configurator.editConfigs', async () => {
        try {
            ...
            const install = await vscode.window.showErrorMessage('未检测到Microsoft C/C++扩展，是否前往安装？', '安装', '取消');
            if (install === '安装') {
    ...
    let setupBuildDebugCmd = vscode.commands.registerCommand('cpp-configurator.setupBuildDebug', async (uri) => {
        ...
        // 打开Webview面板进行配置
        const panel = vscode.window.createWebviewPanel('cppBuildConfig', '配置编译调试文件', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                ...
                case 'browseFile':
                    ...
                    const selectedPaths = await showCustomFilePicker(workspaceFolder.uri.fsPath, message.filter || {}, message.fieldId === 'sourceFiles');
 ...
 * @returns 选中的绝对路径数组
 */
async function showCustomFilePicker(workspaceRoot, filters, isSourceFile) {
    return new Promise(async (resolve) => {
        // 创建Webview面板
        const panel = vscode.window.createWebviewPanel('customFilePicker', '选择文件（支持多选）', vscode.ViewColumn.One, {
            enableScripts: true,
...
function extractDataFromExistingConfigs(tasks, launchConfig) {
    ...
    if (task.args && Array.isArray(task.args)) {
        // 提取源文件
        data.sourceFiles = task.args.filter((arg) => !arg.startsWith('-') &&
            (arg.includes('.c') || arg.includes('.cpp') || arg.includes('.cc') || arg.includes('.cxx'))).map(normalizePath);
        // 提取库文件
        data.libraryFiles = task.args.filter((arg) => !arg.startsWith('-') &&
            (arg.includes('.a') || arg.includes('.o') || arg.includes('.so') ||
                arg.includes('.lib') || arg.includes('.dll'))).map(normalizePath);
        // 提取输出路径
        const outputIndex = task.args.indexOf('-o');
        if (outputIndex !== -1 && outputIndex + 1 < task.args.length) {
...
async function generateConfigFiles(data, workspacePath, cppConfig) {
    ...
    try {
        ...
        fs.writeFileSync(path.join(vscodePath, 'tasks.json'), JSON.stringify(tasksJson, null, 4), 'utf8');
        ...
        fs.writeFileSync(path.join(vscodePath, 'launch.json'), JSON.stringify(launchJson, null, 4), 'utf8');
