...
function getDefaultDebuggerPathFromConfig(cppConfig: any, platform: string, wsl: boolean): string {
...
export function activate(context: vscode.ExtensionContext) {
    ...
    let setupBuildDebugCmd = vscode.commands.registerCommand('cpp-configurator.setupBuildDebug', async (uri: vscode.Uri) => {
        ...
        panel.webview.html = getWebviewContent(
            existingSources, existingLibs, existingOutputName, existingOutputPath,
            existingLaunchConfig, workspaceFolder.uri.fsPath, panel.webview
        );
            ...
            async (message) => {
                switch (message.command) {
                    ...
                    case 'cancel':
                        ...
                        break;
                    case 'browseFile':
                        const currentRelativePaths: string[] = message.currentFiles || [];
...
function toAbsolutePath(relativePath: string, workspacePath: string): string {
...
async function showCustomFilePicker(workspaceRoot: string, filters: any, isSourceFile: boolean, initialSelected: string[] = []): Promise<string[]> {
    return new Promise(async (resolve) => {
        ...
        let selectedFiles: Set<string> = new Set(initialSelected.map(p => normalizePath(p)));
        ...
        async function refreshFileList() {
            try {
                ...
                panel.webview.html = getFilePickerHtml(
                    currentDir, workspaceRoot, fileListHtml,
...
async function handleCppPropertiesChange(filePath: string) {
    ...
    try {
        ...
        if (JSON.stringify(newConfig) === JSON.stringify(currentCppConfig)) return;
...
function extractDataFromExistingConfigs(tasks: any[], launchConfig: any): any {
    ...
    if (!buildTask) buildTask = tasks.find(t => t.args && Array.isArray(t.args) && t.args.length > 0);
    ...
    if (task.args && Array.isArray(task.args)) {
        data.sourceFiles = task.args.filter((arg: string) => !arg.startsWith('-') && (arg.includes('.c') || arg.includes('.cpp') || arg.includes('.cc') || arg.includes('.cxx'))).map(normalizePath);
        data.libraryFiles = task.args.filter((arg: string) => !arg.startsWith('-') && (arg.includes('.a') || arg.includes('.o') || arg.includes('.so') || arg.includes('.lib') || arg.includes('.dll'))).map(normalizePath);
        const outputIndex = task.args.indexOf('-o');
        if (outputIndex !== -1 && outputIndex + 1 < task.args.length) {
    ...
    if (launchConfig.args && Array.isArray(launchConfig.args)) data.args = launchConfig.args;
    return data;
}
...
async function autoSyncConfigs(workspacePath: string): Promise<boolean> {
    ...
    const tasksPath = path.join(workspacePath, '.vscode', 'tasks.json');
    if (!fs.existsSync(cppPropertiesPath) || !fs.existsSync(tasksPath)) return false;
    try {
...
function getWebviewContent(
    existingSources: string[], existingLibs: string[], existingOutputName: string,
    existingOutputPath: string, existingLaunchConfig: any, workspacePath: string, webview: vscode.Webview
): string {
    ...
    if (existingLaunchConfig.environment && Array.isArray(existingLaunchConfig.environment)) {
        existingEnvironment = existingLaunchConfig.environment.map((env: any) => `${env.name}=${env.value}`).join('\n');
    }
    return `<!DOCTYPE html>
<html>
...
function expandIncludePath(includePath: string[], workspacePath: string): string[] {
    ...
    for (const inc of includePath) {
        ...
        if (inc.includes('/**')) {
            ...
            const normalizedBase = normalizePath(basePath);
            if (inc.includes('${workspaceFolder}')) result.push(basePathWithVar);
            else if (path.isAbsolute(basePathWithVar)) result.push(toWorkspaceRelativePathWithDotDot(basePathWithVar, workspacePath));
            else result.push('${workspaceFolder}/' + normalizePath(basePathWithVar));
            if (fs.existsSync(normalizedBase) && fs.statSync(normalizedBase).isDirectory()) {
                const subDirs = getAllSubdirectories(normalizedBase);
                for (const subDir of subDirs) result.push('${workspaceFolder}/' + subDir);
            }
        } else if (inc.includes('${workspaceFolder}') || inc.includes('${default}')) result.push(normalizePath(inc));
        else if (path.isAbsolute(inc)) result.push(toWorkspaceRelativePathWithDotDot(inc, workspacePath));
        else result.push('${workspaceFolder}/' + normalizePath(inc));
    }
...
async function generateConfigFiles(data: any, workspacePath: string, cppConfig: any) {
    ...
    try {
        ...
        if (!compiler) {
            const hasCpp = data.sourceFiles.some((f: string) => f.includes('.cpp') || f.includes('.cc') || f.includes('.cxx'));
        ...
        if (isCppCompiler) {
            if (cppStandard) {
                ...
                if (cppStd.startsWith('c++') || cppStd.startsWith('gnu++')) stdFlag = `-std=${cppStd}`;
            }
        } else {
            if (cStandard) {
                const cStd = cStandard.toLowerCase().replace(/gnu/g, 'gnu');
                if (cStd.startsWith('c') || cStd.startsWith('gnu')) stdFlag = `-std=${cStd}`;
            }
        }
        ...
        defines.forEach((def: string) => { if (def && typeof def === 'string') args.push('-D', def); });
        compilerArgs.forEach((arg: string) => { if (arg && typeof arg === 'string') args.push(arg); });
...
export function deactivate() {
    if (cppPropertiesWatcher) cppPropertiesWatcher.dispose();
}